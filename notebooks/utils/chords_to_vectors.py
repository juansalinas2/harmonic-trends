...
def _pc(note: str) -> int:
    """
    Given note, returns inverval value away from C (going up)
...
def chord_type(token: str, *, default_major: str = "maj") -> str:
    """
    Returns the chord quality/extensions only (no root, no bass).
    Examples:
      "Dmin" -> "min"
      "Cmaj7add9" -> "maj7add9"
      "F#7" -> "7"
    ...
    return rest if rest else default_major

def chord_types_from_string(chord_string: str, *, default_major: str = "maj") -> set[str]:
    # split on whitespace  
    tokens = re.split(r"\s+", chord_string.strip())
    # (optional) keep only chord-like tokens: start with A–G
    tokens = [t for t in tokens if re.match(r"^[A-G]", t)]
    return set(chord_type(t, default_major=default_major) for t in tokens)
...
def chord_token_to_12bit(token: str) -> np.ndarray:
    root, rest, bass = parse_token(token)
...
def chord_string_to_song_matrix(s: str) -> np.ndarray:
    """
    Input: space-separated chord tokens
    Output: (12, L) matrix
    """
    # Handle non-string / empty inputs
    if s is None:
        return np.zeros((12, 0), dtype=np.uint8)
    ...
    # Treat tokens ending with '/' as a stray delimiter: keep the chord (without '/')
    # and keep the next token as its own chord.
    tokens = []
    for t in raw:
        if not t:
            continue
    ...
    # Drop obviously broken tokens
    tokens = [t for t in tokens if t and (not t.startswith("/")) and (not t.endswith("/"))]

    cols = [chord_token_to_12bit(tok) for tok in tokens]
